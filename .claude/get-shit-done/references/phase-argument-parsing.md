...
```bash
PHASE_INFO=$(node "/Users/luca/dev/gpui/helm/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/Users/luca/dev/gpui/helm/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/Users/luca/dev/gpui/helm/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
