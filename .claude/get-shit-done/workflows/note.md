...
- **Global scope**: `/Users/luca/dev/gpui/helm/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/luca/dev/gpui/helm/.claude/notes/`)
...
2. Glob `/Users/luca/dev/gpui/helm/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/luca/dev/gpui/helm/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/luca/dev/gpui/helm/.claude/notes/` — works in any directory
