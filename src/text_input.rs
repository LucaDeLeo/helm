...
impl TextInput {
    ...
    fn submit(&mut self, _: &Submit, _: &mut Window, cx: &mut Context<Self>) {
        if !self.content.is_empty() {
            ...
            cx.emit(TextInputEvent::Submit(text));
        }
    }
...
// Custom element for rendering the text with cursor
...
impl gpui::Element for TextElement {
    ...
    fn paint(
        ...
        let line = prepaint.line.take().expect("line should be set");
        line.paint(
            bounds.origin,
            window.line_height(),
            gpui::TextAlign::Left,
...
pub fn bind_text_input_keys(cx: &mut App) {
    cx.bind_keys([
        ...
        gpui::KeyBinding::new("ctrl-cmd-space", ShowCharacterPalette, Some("TextInput")),
